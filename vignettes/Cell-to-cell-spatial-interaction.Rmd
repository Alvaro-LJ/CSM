...
- [Cell-to-cell spatial interaction analysis](#cell-to-cell-spatial-interaction-analysis)
    ...
    - [Cumulative interaction-based analysis](#cumulative-interaction-base-analysis)
    - [Gcross function](#gcross-function)
    - [Ripley's K](#ripleys-k-function)
- [Multi-level modelling](#multi-level-modelling)
- [Three better than two](#triad-analysis)
- [Measuring barrier effect](#barrier-effect-analysis)
...
## Introduction
Cell-to-cell spatial interaction analyses are a group of techniques aimed at identifying simple cellular interactions.
...
* __[Multi-level modelling](#multi-level-modelling):__ Adds two extra layers of complexity to the analysis. First, it takes
  into account the cellular density of the target cell that can partially explain spatial interactions. Second, it analyzes the 
  influence of image metadata on the spatial interaction. Multi-level modelling imposes a nested data structure to the cell matrix, where
  every COO has an associated spatial interaction with the target cell, 
  while belonging to an image with specific metadata and target cell density.
...
## Test datasets
The test dataset consists of a cell feature matrix obtained from two 500x500 pixels breast carcinoma images. 
...
In this example the distance between CD8 cells (COO) and tumor cells (Target) will be calculated.
...
It can be calculated using the `Cumulative_Interaction_generator()` function. It requires the user to specify the distance sampling
strategy using the 'Start_from', 'Stop_at' and 'Sampling_frequency' function arguments.
...
generated are considered to be sample specific, the average distance of B09 is way higher thant that expected by change, indicating 
repulsion. In contrast, the spatial clustering in B14 is similar to that expected by chance.
...
the image metadata provided has a positive influence in the spatial association. This means that if a COO belongs to 
...
between tumor cells (COO) and the two types of CD8 cells (Target cells) present in our samples.
...
Barrier effect analysis is a special type of triad analysis. It measures the spatial interaction of three cell types but taking into account their spatial dependency.
Barrier effect analysis measures how a cell population interferes with the spatial interaction occurring between two other cell types.

The barrier effect function works in a multi-step manner:

* __1<sup>st</sup>:__ Define Cell of Origin (COO), target cell and distance search. The function will find target cells within the search distance for all COO contained in the image.
* __2<sup>nd</sup>:__ Target cells within distance can be filtered according to tolerance angles. This means that if two target cells are close to each other in taking into account their
angular distance, the closest target cell will be kept in the analysis. This is helpful to precisely compute the space required for the COO to reach it's closest targets. 
* __3<sup>rd</sup>:__ For every COO and it's interacting target cells, compute the area where potential barrier cells (or other molecules) could be interfering with the spatial interaction. This is computed by overlying triangular polygons between the COO and target cells according to a user defined angle.
* __4<sup>th</sup>:__ Quantify the number of barrier cells (or other molecules) that are present within this area.

The `Barrier_effect_calculator()` is the function in charge of doing this computation. This function can compute barrier cells and also compute barrier positive pixels 
(can either work with thresholded or non-thresholded images). The function will return the number of barrier cells or sum of pixel values. In addition, the function will normalize these metrics 
by the total target cells analyzed, the cumulative distance between the COO and target cells and the area analyzed.

If barrier pixels need to be analyzed, the user must provide a named list of parameters indicating the path to the folder containing the images, and any image pre-processing steps required.

```{r Barrier calculator, eval = FALSE, fig.align='center', out.width='100%', fig.width = 10, fig.height = 8}
# Generate the imput directory
...
#Compute the cells and pixels in the barrier (CAN ONLY RUN INTERACTIVELY)
...
Once we have our barrier cell or barrier pixel data generated, it can be convenient to summarize results by sample. The `Barrier_effect_analyzer()` will generate
sample-wise summaries for barrier cells or pixels.

```{r Barrier evaluation, eval = FALSE, fig.align='center', out.width='100%', fig.width = 10, fig.height = 8}
...
Finally, graphs depicting the barrier cell density can also be generated using the `Barrier_graph_maker()`. This function supports including an image in the background, which can be
useful to demonstrate pixel-based barrier effect results.

```{r Barrier plot, eval = FALSE, fig.align='center', out.width='100%', fig.width = 10, fig.height = 8}
Barrier_graph_maker(
  DATA_Phenotypes = CSM_Phenotypecell_test,
  ...
  Image_directory = dir(Input_Dir, full.names = TRUE)[[2]], #Select the appropriate image
  Pixel_distance_ratio = NULL,
  Channel_to_display = 3, #Select the GZMB channel
  Image_rotate = 0,
  Image_x_flip = FALSE,
  Image_y_flip = TRUE
)
...
For more information regarding the interpretation of this metric please see the original paper published at
https://www.nature.com/articles/s41467-023-37822-0.

As for cumulative interaction metrics, the user must decide the sampling strategy with  the 'Gradient_start', 
'Gradient_stop' and G'radient_sampling' function arguments.
...
thresholded images. The analysis is designed to understand how cells are positioned related to non-cellular
elements (usually extracellular matrix elements). In practice, this tool can be used to assess distances from cells
to any sort of biomolecule measured in the tissue.
...
closest positive pixel. Due to discrepancies between cell coordinate system and origina image pixel coordinates that may
occur, image adjustments could be required before proceeding.
The function will show a test image with a cell overlay to check that the alignment is correct before proceeding.
...
cells according to this distance. For example we will classify CD8 cells closer than a 3 pixel distance as being
...
Now the information regarding the position of these cells with respect to CK has been included in the cell phenotype label.
