...
- [Analyzing neighborhood morphology](#morphology-analysis)
...
defining what "near" means. Different algorithms define neighbors in different manners, influencing the final result.
...
* __[Closest neighbor-based approach](#closest-neighbor-based-approach):__ These approaches are based on calculating the
  neighbors for every cell. Afterwards this information is used to identify patterns.
    ...
    * __[UTAG](#utag):__ Based on the original publication from the Elemento group
    ...
    UTAG is interesting because it can perform cell neighborhood analysis without cell phenotype label information.
* __[Tiled-based approach](#tiled-based-approach):__ This approach requires the overlaying a tile grid over the tissue area.
  ...
  However, they are subject to the 'Modifiable areal unit problem' discussed in the 'Calculating tissue heterogeneity' vignette 
...
* __[Global interaction pattern (imcRtools)](#global-interaction-pattern-imcrtools):__ CSM ports imcRtools neighborhood discovery pipeline.
  ...
  interaction score for every cell type pair. Global interaction patterns are identified across samples, not cell types. This approach
...
## Closest neighbor-based approach
...
* __N closest cells:__ The neighbors of any cell in the sample are it's N closest cells, being N an integer number. As N increases,
  ...
  For example, the N closest neighbors to a single cell that is spatially isolated (like a cell stranded in the middle of stroma) will
...
In practice, the choice of neighbor definition and the choice of parameters is arbitrary. Depending on the choice, different types of neighborhoods capturing
various types of spatial patterns can be identified. There is no single ideal way to identify neighborhoods.
...
In the following example we will calculate neighbors, defined as any cell within 30 pixels from the COO. Cells labelled as 'OTHER' will not be visited by the
...
* __COO to visit:__ If the user has some previous information regarding cells of interest, it can be used to ameliorate computational demands.
  The 'COO_to_visit' argument allows the user to select which cells will be visited by the algorithm and considered COO. It should be a logical vector where
  TRUE cells are visited and FALSE cells are ignored. However, ignored cells will also be taken into account as potential neighbors of COO.
* __Message strategy:__ There are two ways the feature information from neighbors can be combined:
    ...
    * __Sum:__ Adds the cell features values alone. Is compatible with proximity weighting (see below). Takes into account the cell density in the neighborhood. It can increase the dinamic 
    range of features.
* __COO weight:__ Message passing can somehow dilute the feature information of the COO. In cases where there are too many neighbors, message passing can
...
* __Neighbor's weights:__ Should all the neighbors share their features equally? Imagine the following scenario: the COO has 10 closest neighbors, 5 within a 10 pixel
  ...
  features is only rarely expressed in the data or shows very low expression values on average. However very few high expressing cells can be encountered from time to time.
  ...
  The function allows both correcting for proximity to COO and by low expression ('disregarded_minority' correction)
    * __Proximity correction:__ Every neighbor is at distance d from the COO. The sum of all distances d from all the neighbors of the COO is
    D. Weight for every neighbor is calculated as the following:
$$
...
After deciding the parameters, message passing can be performed. Here we will apply the same neighbor definition as for conventional closest
neighbors. We will take into account proximity to COO in message passing.
...
If we perform message passing by sum in stead, we would obtain always higher feature values. In addition,
the dinamic range of the features is also higher. 
...
To circunvent this limitation, another approach is to divide the tissue samples into tiles and analyze the amount of tiles where each neighborhood can be found. As described in the
...
to each neighborhood by tile. In addition, it returns the most represented neighborhood by tile. This information can be useful to have an idea of the spatial clustering of neighborhoods.

```{r Tiling_elections, eval = FALSE, fig.align='center', out.width='100%', fig.width = 10, fig.height = 8}
#Can only run interactively
...
This section of the vignette is homologous to that written for unsupervised clustering to indentify cell phenotype labels.
complete unsupervised pipeline may include dimension reduction, data denoising, estimating the optimal number of clusters and finally perform data
...
denoising pipeline until results are adequate. To this end, the `Neighborhood_discovery_function()`, `UTAG_Neighborhood_identifier()` and 
`Tiled_Image_Clustering_function()`(see below) functions provides the option to pre-process the data and return the interim result. 
In order to activate this behavior the user must set the argument `Stop_at_preprocessing = TRUE`. 
This will avoid running into the clustering process every time the function is called. The resulting pre-processed data can be provided to the functions 
...
identify as the explicit number of clusters to obtain (`Max_SOM_neighborhoods`, `Max_N_neighborhoods_Batch`, `Max_N_neighborhoods_GMM`, `Max_N_neighborhoods_CLARA`).


## Tiled-based approach
This strategy first divides the tissue into tiles. Each tile is considered an individual neighborhood. Then cell composition among neighborhoods is analyzed.
First, tiled images are generated. Then, patterns are identified by using the `Tiled_Image_Clustering_function()`.

```{r Tiled_clustering, eval = FALSE, fig.align='center', out.width='100%', fig.width = 10, fig.height = 8}
#Can only run interactively
...
#Modify names
Neighborhood_tiles_result <- Clustered_Tiled_Images_renamer(Tiled_images = Neighborhood_tiles,
                                                            New_names = c("Depleted_neighborhood",
                                                                          "OTHER_RICH",
                                                                          "IMMUNE_NICHE",
                                                                          "TUMOR_DENSE",
                                                                          "TUMOR_LIGHT"))
...
Once the neighborhoods have been identified, quantification and analysis can be performed using `Clustered_Tiled_Images_analyzer()`.
In addition, neighborhood spatial heterogeneity can be analyzed by calculating several heterogeneity indexes and graph modularity.
```{r Tiled_clustering_analysis, eval = FALSE, fig.align='center', out.width='100%', fig.width = 10, fig.height = 8}
#Can only run interactively
Clustered_Tiled_Images_analysis <- Clustered_Tiled_Images_analyzer(Tiled_images = Neighborhood_tiles_result, 
                                                                   Perform_heterogeneity_analysis = T, 
                                                                   Graph_Modularity_resolution = 0.5 
)
```

Finally, the results can be graphicated.
```{r Tiled_clustering_graphs, eval = FALSE, fig.align='center', out.width='100%', fig.width = 10, fig.height = 8}
Clustered_Tiled_Images_graphicator(Tiled_images = Neighborhood_tiles_result,
...
## Morphology analysis
Cells and cellular neighborhood may organize themselves into discrete tissue structures. For example, lymphoid follicles in lymph nodes are discrete spheric to oval tissue structures.
CSM implements a tool to identify and measure morphological properties (size, shape...) of discrete tissue structures. Although described here, morphology analysis can be performed for
single cell types in addition to cellular neighborhoods.

In order two perform morphology analysis discrete tissue structures must be present. The morphology analysis implemented in CSM requires a watershed-based segmentation step. If target tissue
structures are ill-defined, they wont be spotted by the algorithm. In addition, if tissue structures show extreme morphological variations (striking differences in size or shape), no single 
watershed algorithm will be able to detect all structures at once.

The analysis works in the following way:

* __1<sup>st</sup>:__ Generate a tiled version of your cell data (using the `Image_tiling_processing_function()` or data generated using the `Tiled_Image_Clustering_function()`). The user
must also decided the neighborhood-type that is going to be analyzed.
* __2<sup>nd</sup>:__ Identify tissue structures using the `Structure_morphometry_calculator()`. It will require the user to specify the minimum number of tiles required to consider
a tissue structure as real, and the minimum number of cells belonging to the desired neighborhood that must be present in each tile to consider the tile as 'positive'. Other parameters 
conditioning the segmentation result must also be specified (smoothening, tolerance and minimum neighbor distance).
* __3<sup>rd</sup>:__ The results can be summarized by sample using the `Structure_morphometry_analyzer()`. In addition, tissue structures can be plotted according to desired 
features using the `Structure_morphometry_graph_maker()`.

First datasets containing cellular neighborhood labels must be arranged into tiles.
```{r Neighborhoods into tiles, eval = FALSE, fig.align='center', out.width='100%', fig.width = 10, fig.height = 8}
Tiled_images <- Image_tiling_processing_function(N_cores = 1,
                                                 DATA = DATA_Neighborhoods_names,
                                                 Tile_width = 25,
                                                 Tile_height = 25,
...
Then, structures can be identified. In this example we will try to find tissue structures composed of CD8 negative rich areas.
```{r Tissue structures, eval = FALSE, fig.align='center', out.width='100%', fig.width = 10, fig.height = 8}
Structures <-
  Structure_morphometry_calculator(N_cores = 1,

                                   Tiled_Images = Tiled_images,
                                   Variable_name = "Neighborhood_assignment",
                                   Feature_to_analyze = "CD8_NEG_RICH",
...
Finally, identified structures can be summarized and graphs can be generated.

```{r Tissue structure analysis, eval = FALSE, fig.align='center', out.width='100%', fig.width = 10, fig.height = 8}
#Analyze tissue structure
Structure_morphometry_analyzer(DATA = Structures)

#Graph and color structures according to structure ID
...
#Graph and color structures according to structure perimeter size
Structure_morphometry_graph_maker(Tiled_Images = Tiled_images,
                                  Variable_name = "Neighborhood_assignment",
                                  DATA_Morphometry = Structures,
                                  Color_by = "s.perimeter",
                                  Image_name = "ABCM22001_B14_MiniCrop.tif")

#Graph and color structures according to structure sphericity
Structure_morphometry_graph_maker(Tiled_Images = Tiled_images,
                                  Variable_name = "Neighborhood_assignment",
                                  DATA_Morphometry = Structures,
                                  Color_by = "m.eccentricity",
                                  Image_name = "ABCM22001_B14_MiniCrop.tif")
```

## Global interaction pattern-imcRtools
This approach is based on counting the spatial interaction between cell type pairs based on the `imcRtools::countInteractions()` function. After analyzing sample-wise cell pair interactions, sample-wise clustering can be performed to identify recurrent patterns.
This approach can be applied using the `Interaction_counter()`. Then interaction patterns can be analyzed and identified using the `Interaction_analyzer()`.
In this case, since we only have two samples, samples cannot be clustered.

```{r Spatial_interactions, eval = FALSE, fig.align='center', out.width='100%', fig.width = 10, fig.height = 8}
#Can only run interactively

#Compute interactions
Interaction_by_sample <-
  Interaction_counter(
    DATA = CSM_Phenotypecell_test,
    Phenotypes_included = unique(CSM_Phenotypecell_test$Phenotype),
    N_cores = 1,
...
## SPIAT-based
This approach is based on the SPIAT function `SPIAT::identify_neighborhoods()`. It finds neighborhoods within each sample. Although useful, this approach makes comparing
neighborhoods across samples dificult.

```{r SPIAT_Clustering, eval = FALSE, fig.align='center', out.width='100%', fig.width = 10, fig.height = 8}
#Can only run interactively
DATA_SPIAT <-
