...
Finally results can be graphicated.
...
First tiled images need to be generated. Then patterns are identified by using the `Tiled_Image_Clustering_function()`.
...
In addition, neighborhood spatial heterogeneity can be analyzed using this function using by calculating several heterogeneity indexes and graph modularity.
...
This approach is based on counting the spatial interaction between cell type pairs based on the `imcRtools::countInteractions()` function. Then global sample patterns are identified.
...
In this case, since we only have two samples, clustering samples cannot be computed.
