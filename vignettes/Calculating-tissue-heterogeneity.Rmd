...
- [Spatial heterogeneity](#spatial-heterogeneity)
    ...
    - [Estimating MAUP](#estimating-maup)
...
Broadly, CSM heterogeneity analysis tools can be divided into GLOBAL heterogeneity (does not account for
...
* __Richness and evenness metrics__: Includes Shannon, Simpson, Inverse Simpson, Renyi entropy,
  ...
  between cell types). Different metrics combine these two factors in different ways to obtain the final metric value.
* __Evenness only metrics__: Gini index is the most representative of this type. Gini index analyzes the balance of the relative 
  abundance of cell types in the sample. Unbalacend cell type distributions give rise to a Gini index value close to 1. Balanced cell
  composition result in values close to 0.5.
* __Observed vs Expected distribution metrics__: Includes Kullback-Leibler and Jensen-Shannon indexes. These metrics may quantify
  the deviation from perfect balance between cell types or the deviation of each individual sample cell composition from the expected composition according
...
to understand that the number of phenotypes included in the analysis will heavily influence the final results if using a Richness and evenness metric.
More cell types will necessarily result in higher heterogeneity.
...
heterogeneity in three different ways:

- **[Tile-based](#tile-based):** This approach divides the image into tiles and computes heterogeneity metrics
  within these tiles. It can capture spatially restricted hot or cold spots. In addition, CSM implements tools
...
- **[Texture features](#texture-features):** Instead of cell composition, texture features analyzes the spatial distribution
  ...
  tile. This abundance by tile information is used to calculate the Grey-level co-ocurrence matrix (GLCM). GLCM is then used to
  compute the average texture features. Texture features reflect different aspects of spatial distribution heterogeneity.
...
The choice of the approach will depend on the scientific question at hand. Tiled based approaches provide information of local heterogeneity
...
on the result. Aggregating data into tiles is associated with an inherent bias usually referred to as 'Modifiable areal unit problem'
...
from tiling their data just because there is a potential bias. Just take into account this potential bias. Therefore, it is always a good idea to check how
tiling strategies may modify the final conclusions of the analysis (please see [Estimating MAUP](#estimating-maup) for more information).
...
#Split the smallest image into 5 columns and 5 rows
...
Tiled_images[1]
```
Every element in the list represents a single image. Every element is also a list with two tibbles. The first contains tile information. 
The second contains cell tile assignment.
...
The first analysis approach is to set an arbitrary threshold and quantify the number of tiles by sample that are above
the threshold ('Quantify_by_Threshold' approach). The second approach computes various central tendency and dispersion metrics of
tile heterogeneity results ('Overall_Summary' approach). Both methods can compute the spatial clustering of hot-spot tiles using
Moran's I.
...
The results can also be plotted. The `Tiled_images_graphicator()` will generate cell counts by tile for the user selected cell phenotypes.
...
CSM implements several tools to evaluate the influence of the Modifiable areal unit problem (MAUP) in the final result of spatial heterogeneity metrics (both tile-based and texture features).
These estimations may aid the user to judge if results are robust or are unstable if minor modifications to the tiling parameters are made. MAUP estimation should not be interpreted
as the confidence interval of heterogeneity metrics. The method described below has nothing to do with generalization of metrics from sample to global population.

The MAUP estimation process implemented in CSM works in several steps:

* __1<sup>st</sup>:__ Cell-data with phenotype labels will be tiled according to several randomly generated parameters. Parameters include the tile width and height, and cell coordinate
rotation and flipping. The user may select the amount of random variation of tile width and height. The larger the number of random iterations requested, the higher the computational burden.
Of note the tiled cell-data iterations generated will only differ in their tiling strategy parameters. Cell-data information is __kept intact__ during the whole process.
* __2<sup>nd</sup>:__ Compute metrics using these tiled data. Either heterogeneity by tile or texture features.
* __3<sup>rd</sup>:__ Analyze the data summarizing the results for every sample. 

As an example we can generate 10 random tile parameter iterations.

```{r Random Tiles, eval = FALSE, fig.align='center', out.width='100%', fig.width=6, fig.height=4}
...
Afterwards, these tiled data structures can be analyzed. For example we can first compute heterogeneity metrics for every tile and then analyze the results using the 
`Random_tiled_image_heterogeneity_calculator()` and the `Random_tiled_image_heterogeneity_analyzer()`.

```{r Random heterogeneity, eval = FALSE, fig.align='center', out.width='100%', fig.width=6, fig.height=4}
...
In addition, it is also possible to compute the texture features in these tiled structures using the `Random_texture_features_calculator()`.
```{r Random texture, eval = FALSE, fig.align='center', out.width='100%', fig.width=6, fig.height=4}
