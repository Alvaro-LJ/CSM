...
This vignette explains how CSM works to perform pixel-based analyses.
Pixel analysis may be required to assess extra-cellular features (for example measuring collagen proteins).
There are two main approaches to perform pixel analysis in CSM. First, pixels can be thresholded followed by quantification. 
...
Any background pixel will be ignored in the analysis.

Pixel analysis in CSM has been developed using the TIFF image format. However, other supported formats include
png and jpeg.
...
Tissue mask pixels can be obtained by using all or a subset of image channels. These channels are pooled together by adding up their pixel
values and then dividing the result by the total number of channels used. Three methods can be used to obtain the tissue mask:
...
* Absolute: Any pixel with a value above 0 will be considered to be a foreground pixel. Works well for images with very little noise.
...
For non-arbitrary approaches, thresholds can be computed locally (a unique threshold is calculated for every image) or globally (a single threshold for all the image in the directory provided). Global thresholds are usually preferred as local thresholds can 
capture image specific patterns that are not generizable. However, global thresholding is more computationally demanding (it requires
loading all the images in the directory and storing their pixel values to compute the global threshold).

Before thresholding the pixels (either for tissue mask generation or target channel thresholding), Gaussian blur can be applied. This can smooth borders of tissue masks and reduce salt and pepper noise of the target channel before proceeding.
...
The app includes a control panel to control the tissue mask parameters  and the thresholding parameters (see above). If Multilevel thresholding with arbitrary cut-off points is 
required, the user can write a vector containing the cut-off values in the `Value` text box. Once the parameters are OK the user save the parameter list into the global environment using the
`Download Parameters` button.

```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("Figures/Pixel_thresholding_app.png")
```
...
thresholded images generated in the process which can be used to combine the pixel information with cell spatial location information 
(see below and `vignette("Cell-to-cell-spatial-interaction")`).
...
threshold. Blurring will only be applied to the tissue mask generation process. If the user has already generated a parameter list using the
`Image_thresholding_app_launcher()` it can be provided to the `Pixel_Threshold_calculator()` function using the `Parameter_list` argument. If a parameter list is provided,
any other thresholding related arguments will be ignored.
...
The `Cell_to_pixel_distance_calculator()` will allow users to calculate the distance between cells and the closest positive pixel.
...
Before computing the actual distances, the function will take the image with the smallest number of cells to run a test.
Use the test to check if image needs to be 
rotated or flipped or if 'pixel_distance_ratio' needs to be adjusted. If satisfied, interact with the menu to proceed with the computation.
...
By default the MFI is calculated taking into account foreground pixels. In addition to foreground pixels masks, users can define 
specific tissue compartments where MFI should be analyzed. Compartments are generated using single or multiple image channels that are
thresholded following similar approaches as for tissue mask. These are provided as a list to the 'Target_masks' argument.

In the following example the MFI of GZMB is calculated in a the CK-EPCAM and CD8a tissue compartment. This will approximate the 
mean GZMB expression of CD8a lymphocytes that reside in the tumor.
...
Binary thresholded images (not multilevel thresholded) can be combined using the `Binary_threshold_image_combinator()` function.
