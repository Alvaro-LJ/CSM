...
## Introduction
Cell feature data is obtained from images. Images are usually subject to inherent batch effect that can be removed using
normalization procedures. Normalization can be performed at the image level (not supported currently by CSM), or at the cell
feature expression level. While it is certainly possible that normalization can remove true signal, the benefit of removing
batch effect usually outweigh the drawbacks. This is specially true for noisy features, like the ones derived from immunofluorescence
...
## Test datasets
The test dataset includes cell feature matrix obtained from two 500x500 breast carcinoma images. They have been stained with a multiplexed
...
mxnorm normalization supports both COMBAT and registration algorithms. Users are directed to the associated publication for further information
(https://academic.oup.com/bioinformatics/article/38/6/1700/6496920/). mxnorm allows to take into account two sources of variability:
...
As for mxnorm, a parameters list needs to be created first. simpleSeg normalization allows the use of multiple methods simultaneously.
...
Once you normalize feature data it's a good idea to check that normalization actually did something to your data. 
In CSM you check the effect by running `Normalization_diagnostics()`. Normalization diagnostics returns two analyses.
...
Second, feature expression distribution in all the cells contained in a dataset may be closer to a bimodal distribution or to the normal 
