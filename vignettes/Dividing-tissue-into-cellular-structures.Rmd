...
Dividing the tissue into cellular structures can be useful to locate cells within specific tissue compartments. 
In tumor spatial biology, tissue structuring is commonly used to divide samples into tumor and stromal areas. 
This is why functions described in this vignette are named as "Tumor_Stroma_identifiers".

However, this technique can be used to identify tissue compartments of any type. 
Tissue structures are based on the spatial distribution of a single cell type. 
As far as this cell type shows a good degree of spatial clustering, tissue structuring can be successful.
...
## Quick structuring
This structuring function is based on tiles. It divides areas into tumor and stromal compartments based on presence of tumor cells 
in these tiles. If enough tumor cells are located in a tile, the tile is considered to be a tumor tile. Otherwise the tile is considered to 
belong to stromal tissue. After obtaining tile labels, cells are assigned to a compartment based on tile location. The algorithm is
computationally efficient but results can be chunky if tile size is too big.
...
## DBSCAN structuring
This approach is based on Density Based clustering. 
First, the spatial coordinates of the index phenotype cells (usually tumor cells) are used to identify spatially clustered cells. 
Afterwards, all cells are projected over this clustering. Cells within clusters are considered to be inside tumor nests. Cells outside are considered to be in the stromal compartment. Regarding computation requirements, this method is somewhere in between the tiling strategy
and LISA clustering.
...
## LISA structuring
This approach is based on Local Indicators of Spatial Association. It can be computationally intensive. However,
this method is capable of computing tumor, stromal and border compartments.
...
Since it calculates a single polygon, it will under-perform in highly dispersed index cell scenarios. However, it is faster than
LISA clustering and also allows the user to compute a border compartment. In addition, it will return the area occupied by the
tumor compartment, allowing subsequent density estimation in the tumor compartment using the `Compartment_Phenotype_quantifier()` function.
...
This approach is based on the `SPIAT::define_structure()` function. It can also be very computationally intensive. It can compute a highly granulated compartments (for example dividing the border compartment into inner and outer areas). Results can be simplified by turning 
the 'Simplify_result' to TRUE.
...
Tissue can also be structured using thresholded pixel information. Cells can be assigned to a tissue structure based on their distance to
positive thresholded pixels.
In the following example, TUMOR/STROMAL compartments are defined by a CK-epcam thresholded image. 
This example was also presented in the Cell-to-cell-spatial-interaction vignette (please see `vignette("Cell-to-cell-spatial-interaction")`).
