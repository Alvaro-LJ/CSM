...
Barrier_graph_maker <- function(
  ...
  if(!all(Phenotypes_included %in% unique(DATA_Phenotypes$Phenotype))) stop(paste0("Phenotypes_included must be any of the following: ",
                                                                                   str_c(unique(DATA_Phenotypes$Phenotype), collapse = ", ")))
  ...
  Polygon_list <- map(1:nrow(Barrier_DATA), function(Row_index) Barrier_DATA[[Row_index, "Polygon_objects"]][[1]][['Unified_polygons']])
