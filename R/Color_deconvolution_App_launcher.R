...
#'
#' Initial Pre-processing allows the initial modification of the image to improve results. The "colors" option allows the user to limit the maximum amount of different colors in the image.
#'
#' Color targeting allows different options:
#'\itemize{
#' \item{Color: The user can select the target color by it's HEX code.}
#' \item{RED GREEN BLUE: The user can select the target color by providing 8-bit RGB values.}
...
#' Image Panels:
#'\itemize{
#' \item{Upper left: Image being color processed. Use it to zoom in.}
#' \item{Upper right: Pixel selector. Initial Pre-processing steps will be applied. Use it to select target color.}
